              ...
              {selectedItem.item.type === 'lesson' ? (
                isModifying ? (
                  <div className="p-5 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-2xl space-y-4 animate-in fade-in zoom-in-95">
                    <div className="flex gap-4 border-b border-orange-200/50 dark:border-orange-800/50 pb-3">
                      <label className="flex items-center gap-2 text-sm font-bold text-orange-900 dark:text-orange-300 cursor-pointer"><input type="radio" aria-label="다른 날짜로 이동" checked={modifyType === 'move'} onChange={() => setModifyType('move')} className="w-4 h-4 text-orange-600 bg-white dark:bg-slate-900 border-orange-300 focus:ring-orange-500" /> 이동 (결강+보강)</label>
                      <label className="flex items-center gap-2 text-sm font-bold text-orange-900 dark:text-orange-300 cursor-pointer"><input type="radio" aria-label="이 시간 단순 결강" checked={modifyType === 'cancel'} onChange={() => setModifyType('cancel')} className="w-4 h-4 text-orange-600 bg-white dark:bg-slate-900 border-orange-300 focus:ring-orange-500" /> 단순 결강</label>
                    </div>
                    {modifyType === 'move' && (
                      <div>
                  ...
                  <button onClick={() => { setIsModifying(true); setTargetDate(''); setTargetPeriod(1); }} className="w-full py-4 bg-orange-100 dark:bg-orange-900/30 text-orange-800 dark:text-orange-400 font-bold text-sm rounded-2xl hover:bg-orange-200 border border-orange-200 dark:border-orange-800 transition-colors flex items-center justify-center gap-2">
      ...
      <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-3xl shadow-xl p-6 md:p-8 animate-in zoom-in-95 border border-slate-100 dark:border-slate-700">
        ...
        <div className="space-y-4 text-sm">
          <div className="flex gap-2 bg-slate-100 dark:bg-slate-900 p-1.5 rounded-xl">
            <button aria-label="수업 취소 또는 제외 탭" onClick={() => setNewEventType('exception')} className={`flex-1 py-2.5 rounded-lg font-bold transition-colors ${newEventType === 'exception' ? 'bg-white dark:bg-slate-800 shadow-sm text-orange-600 dark:text-orange-400' : 'text-slate-500'}`}>수업 제외</button>
            <button aria-label="보강 수업 추가 탭" onClick={() => setNewEventType('extra')} className={`flex-1 py-2.5 rounded-lg font-bold transition-colors ${newEventType === 'extra' ? 'bg-white dark:bg-slate-800 shadow-sm text-indigo-600 dark:text-indigo-400' : 'text-slate-500'}`}>보강 추가</button>
          </div>
          <div><label className="block font-bold text-gray-700 dark:text-gray-300 mb-1.5">{newEventType === 'extra' ? '보강 날짜' : '취소 날짜'}</label><input type="date" aria-label={newEventType === 'extra' ? '보강 날짜' : '취소 날짜'} value={newEventDate} onChange={e => setNewEventDate(e.target.value)} className="w-full border border-gray-300 dark:border-slate-600 p-3 rounded-xl bg-white dark:bg-slate-900 text-gray-900 dark:text-white font-bold outline-none focus:ring-2 focus:ring-indigo-500" /></div>
          <div><label className="block font-bold text-gray-700 dark:text-gray-300 mb-1.5">{newEventType === 'extra' ? '보강 교시' : '취소 교시'}</label><select aria-label={newEventType === 'extra' ? '보강 교시' : '취소 교시'} value={newEventPeriod} onChange={e => setNewEventPeriod(Number(e.target.value))} className="w-full border border-gray-300 dark:border-slate-600 p-3 rounded-xl bg-white dark:bg-slate-900 text-gray-900 dark:text-white font-bold outline-none focus:ring-2 focus:ring-indigo-500">{[1,2,3,4,5,6,7].map(p => <option key={p} value={p}>{p}교시</option>)}</select></div>
          {newEventType === 'exception' && <div><label className="block font-bold text-gray-700 dark:text-gray-300 mb-1.5">제외 사유</label><input type="text" aria-label="취소 사유 입력" value={newEventTitle} onChange={e => setNewEventTitle(e.target.value)} className="w-full border border-gray-300 dark:border-slate-600 p-3 rounded-xl bg-white dark:bg-slate-900 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-orange-500" placeholder="행사명, 출장 등" /></div>}
        </div>
        <div className="mt-6 flex justify-end gap-2">
          <button onClick={onClose} className="px-5 py-2.5 bg-gray-100 dark:bg-slate-700 text-gray-700 dark:text-slate-200 rounded-xl text-sm font-bold">취소</button>
          <button onClick={handleAdd} className={`px-5 py-2.5 text-white rounded-xl text-sm font-bold ${newEventType === 'extra' ? 'bg-indigo-600' : 'bg-orange-500'}`}>저장</button>
        </div>
      </div>
    </div>
  );
              ...
              <div className="bg-slate-100 dark:bg-slate-800/50 p-5 md:p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col">
                ...
                <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                  {activeClassEvents.length > 0 ? activeClassEvents.map(e => (
                    <div key={e.id} className={`flex flex-col gap-1.5 p-4 rounded-xl border shadow-sm ${e.type === 'extra' ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-100 dark:border-indigo-800/60' : 'bg-white dark:bg-slate-800 border-orange-200 dark:border-orange-800/60'}`}>
                      <div className="flex justify-between items-center">
                        <span className={`text-xs font-black ${e.type === 'extra' ? 'text-indigo-800 dark:text-indigo-300' : 'text-orange-800 dark:text-orange-300'}`}>{e.date}</span>
                        <div className="flex gap-1.5">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${e.type === 'extra' ? 'bg-indigo-200 dark:bg-indigo-800/60 text-indigo-900 dark:text-indigo-200' : 'bg-orange-100 dark:bg-orange-900/40 text-orange-800 dark:text-orange-200'}`}>{e.period}교시</span>
                          <span className={`text-[10px] font-black px-2 py-0.5 rounded-md text-white ${e.type === 'extra' ? 'bg-indigo-500' : 'bg-orange-500'}`}>{e.type === 'extra' ? '보강' : '결강'}</span>
                        </div>
                      </div>
                      <span className={`text-sm font-bold ${e.type === 'extra' ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-700 dark:text-slate-200'}`}>{e.title}</span>
